...
func loginMethodFromContext(c *gin.Context) string {
	...
	case "/api/oauth/:provider":
		...
		return "oauth"
	case "/api/bootstrap/cc-switch/claim":
		return "cc_switch"
	default:
		return "unknown"
	}
...
func UpdateSelf(c *gin.Context) {
	...
	// CC Switch 认领链路：匿名用户无原始密码，凭一次性 profile_setup_token 设置初始密码。
	if errors.Is(err, errUserPasswordUnset) {
		if flowToken, ok := requestData["profile_setup_token"].(string); ok && flowToken != "" {
			if _, flowErr := model.ConsumeAuthFlow(flowToken, model.AuthFlowMatch{
