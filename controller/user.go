...
func UpdateSelf(c *gin.Context) {
	...
	usedCCSwitchProfileSetup := false
	if errors.Is(err, errUserPasswordUnset) {
		claimUserID, ok := sessions.Default(c).Get(ccSwitchProfileSetupSessionUserIDKey).(int)
		if ok && claimUserID == cleanUser.Id {
			updatePassword = true
			err = nil
			usedCCSwitchProfileSetup = true
		}
	}
	if err != nil {
		if errors.Is(err, errUserPasswordUnset) {
	...
	if usedCCSwitchProfileSetup {
		session := sessions.Default(c)
		session.Delete(ccSwitchProfileSetupSessionUserIDKey)
